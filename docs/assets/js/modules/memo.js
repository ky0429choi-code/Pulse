...
export async function renderMemo({ siteId, date }){
  ...
  root.innerHTML = card("Memo", `
    ...
    <textarea class="textarea" id="memoContent" placeholder="Write a memo..."></textarea>
  ...
  async function loadList(){
    try{
      const listRes = await apiGet("getMemoList", { siteId, limit: 20 });
  ...
  document.getElementById("memoSaveBtn").addEventListener("click", async ()=>{
    ...
    if(!content) return alert("Enter memo content.");
